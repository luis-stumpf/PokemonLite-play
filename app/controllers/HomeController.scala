...
class HomeController @Inject()(val controllerComponents: ControllerComponents) extends BaseController {
    ...
    def initPokemons(): Action[AnyContent] = Action { request =>
        val player1Pokemon = request.body.asFormUrlEncoded.get("player1Pokemon").mkString
        val player2Pokemon = request.body.asFormUrlEncoded.get("player2Pokemon").mkString

        controller.addPokemons(player1Pokemon)
        controller.addPokemons(player2Pokemon)
